...
void BuildMapMenu()
{
    ...
    if (g_Cvar_ExcludeOld.BoolValue) {
        ...
        char exMap[PLATFORM_MAX_PATH];
        for (int i = 0; i < excludeMaps.Length; i++) {
            excludeMaps.GetString(i, exMap, sizeof(exMap));
            g_mapTrie.SetValue(exMap, MAPSTATUS_DISABLED|MAPSTATUS_EXCLUDE_PREVIOUS);
        }
        delete excludeMaps;
    }
    ...
    if (g_Cvar_ExcludeCurrent.BoolValue) {
        ...
        g_mapTrie.SetValue(currentMap, MAPSTATUS_DISABLED|MAPSTATUS_EXCLUDE_CURRENT);
    }
