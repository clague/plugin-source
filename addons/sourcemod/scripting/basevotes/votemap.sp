...
{
    Handle map_array = INVALID_HANDLE;
    if ((map_array = GetMapList()) == INVALID_HANDLE) {
        return 0;
    }
    if (IsValidHandle(g_map_array) && map_array != g_map_array) {
        delete g_map_array;
    }
    g_map_array = map_array;

    Handle display_name_list = GetMapDisplayNameList();
    
    menu.RemoveAllItems();
    
    char map_name[PLATFORM_MAX_PATH], displayName[PLATFORM_MAX_PATH];
    int map_count = GetArraySize(g_map_array);
    
    for (int i = 0; i < map_count; i++)
    {
        GetArrayString(g_map_array, i, map_name, sizeof(map_name));
        GetArrayString(display_name_list, i, displayName, sizeof(displayName));
