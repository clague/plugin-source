...
public int Handle_VoteMenu(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_VoteCancel: CPrintToChatAll("{green}[系统]{white}投票已经取消！");
		case MenuAction_VoteEnd:
		{
			...
			if (param1 == 0 && FloatCompare(GetVotePercent(votes, totalVotes), VOTE_LIMIT) > 0)
			{
				...
				CPrintToChatAll("{green}[系统]{white}已成功切换无限体力参数！");
			}	
		}
	}
}
...
public Action DoVoteMenu(int client, int args)
{
	if (IsVoteInProgress())
	{
		CPrintToChat(client, "{green}[系统]{red}当前有投票正在进行！");
		return;
	}
	
	if (!IsPlayerAlive(client))
	{
		CPrintToChat(client, "{green}[系统]{red}只有活着的人才能发起投票！");
		return;
	}

	if(hPKMode.BoolValue)
	{
		CPrintToChat(client, "{green}[系统]{red}当前地图不允许改变参数！");
		return;
	}
	...
	CPrintToChatAll("{green}[系统]{white} %s 投票更改无限体力参数！", name);
}
	...
	switch (action)
	{
		...
		case MenuAction_VoteEnd:
		{
			...
			if (param1 == 0 && FloatCompare(GetVotePercent(votes, totalVotes), VOTE_LIMIT) > 0)
			{
				...
				CPrintToChatAll("{green}[系统]{white}已成功切换全砍刀模式！");
			}	
		}
	}
}
...
public Action DoVoteMenu2(int client, int args)
{
	...
	char name[500];
	FetchColoredName(client, name, sizeof(name));
	CPrintToChatAll("{green}[系统]{white} %s 投票更改全砍刀模式！", name);
}
